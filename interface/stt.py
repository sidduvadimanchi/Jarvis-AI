...
PREFERRED_MIC_NAME: str = (os.getenv("PreferredMicName") or os.getenv("MIC_DEVICE_NAME") or "").strip()
PREFERRED_MIC_INDEX_RAW: str = (os.getenv("PreferredMicIndex") or os.getenv("MIC_DEVICE_INDEX") or "").strip()
STT_VERBOSE: bool = (os.getenv("STT_VERBOSE", "0").strip().lower() in {"1", "true", "yes", "on"})

# BUG-14 FIX: pathlib everywhere
_ROOT       = Path.cwd()
_DATA_DIR   = _ROOT / "Data"
_TEMP_DIR   = _ROOT / "data" / "Files"
...
CALIBRATE_SECS     : float = 0.5   # ambient noise calibration duration
ENERGY_THRESHOLD   : int   = 300   # mic sensitivity tuned for normal speaking volume
...
def _log(message: str, *, verbose_only: bool = False) -> None:
    if verbose_only and not STT_VERBOSE:
        return
    print(message)
...
def _calibrate(source: object) -> None:
    ...
    try:
        ...
        _log(f"[STT] Calibrated. Energy threshold: {_recognizer.energy_threshold:.0f}", verbose_only=True)
    except Exception:
        pass
...
def _find_mic_index() -> Optional[int]:
    ...
    if PREFERRED_MIC_INDEX_RAW:
        try:
            preferred_index = int(PREFERRED_MIC_INDEX_RAW)
        except ValueError:
            _log(f"[STT] Invalid PreferredMicIndex '{PREFERRED_MIC_INDEX_RAW}'.", verbose_only=True)
    ...
    if not input_devices:
        _log("[STT-Primary] No input devices found by PyAudio.")
        return None

    if STT_VERBOSE:
        _log(f"[STT] Found {len(input_devices)} input device(s):")
    ...
    if preferred_index is not None:
        for idx, name in input_devices:
            if idx == preferred_index:
                _log(f"[STT] Preferred mic index selected: {idx} -> {name}")
                return idx

    if PREFERRED_MIC_NAME:
        wanted = PREFERRED_MIC_NAME.lower()
        for idx, name in input_devices:
            if wanted in name.lower():
                _log(f"[STT] Preferred mic name matched: {idx} -> {name}")
                return idx
    ...
    def priority_score(item: tuple[int, str]) -> int:
        ...
        if default_index is not None and item[0] == default_index:
            return -100
    ...
    if working:
        ...
        _log(f"[STT] Selected mic index {best_idx}: {best_name}", verbose_only=True)
        return best_idx

    if default_index is not None:
        _log(f"[STT] Falling back to Windows default input device index {default_index}")
        return default_index

    _log("[STT] No working microphone detected.")
    return None
...
def _primary_listen() -> Optional[str]:
    ...
    try:
        ...
        _log(f"[STT-Primary] Google API error: {e}")
        return None
    except Exception as e:
        _log(f"[STT-Primary] Recognise error: {e}")
        return None
...
def SpeechRecognition() -> Optional[str]:  # BUG-13 FIX: type hint
    ...
    if _SR_AVAILABLE:
        raw = _primary_listen()
        if raw:
            _log(f"[STT] Primary: {raw!r}", verbose_only=True)
    ...
    if result:
        _log(f"[STT] Final: {result!r}", verbose_only=True)
    return result or None
